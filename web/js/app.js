...
function toggleWorkspaceMode() {
    ...
    if(useWs) {
        prefix.classList.remove('hidden'); input.classList.remove('rounded-l-md'); input.classList.add('border-l-0');
        ...
        prefix.classList.add('hidden'); input.classList.add('rounded-l-md'); input.classList.remove('border-l-0');
    ...
    try {
        ...
        if (data.path && data.path !== "") {
            ...
            if (handleWorkspace) {
                ...
                if (wsCheckbox.checked) { wsCheckbox.checked = false; toggleWorkspaceMode(); }
            }
        }
...
async function saveSettings() {
    ...
    const newSettings = { workspace: globalWorkspace, log_buffer: false, global_env: globalEnv };
    ...
    try {
        ...
        if (bulkContainer) {
            if (currentTagFilter !== null) {
                ...
                bulkContainer.classList.add('hidden'); bulkContainer.classList.remove('flex');
            }
        }
...
async function submitEnv(e) {
    ...
    try {
        ...
        if (res.ok) { closeEnvModal(); showToast(".env saved securely!", "success"); } 
...
function toggleBoard() {
    const list = document.getElementById('tag-list'); const chevron = document.getElementById('board-chevron');
    if(list) list.classList.toggle('hidden'); if(chevron) chevron.classList.toggle('rotate-180');
}
