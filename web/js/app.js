...
function showContextMenu(e, pId, pName, status) {
    ...
    items.forEach(item => {
        ...
        btn.innerHTML = `
            <svg class="w-4 h-4 ${item.color} group-hover:scale-110 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function handleCmdSearch(e) {
    ...
    cachedProjects.forEach(p => {
        const safeId = p.id || p.ID;
        const safeName = p.name || p.Name || "Unknown";
        
        allActions.push({ name: `Start Project: ${safeName}`, searchKey: `startproject${safeName.toLowerCase()}`, icon: "▶️", action: () => { startProject(safeId, safeName, null); }});
        allActions.push({ name: `Stop Project: ${safeName}`, searchKey: `stopproject${safeName.toLowerCase()}`, icon: "⏹️", action: () => { stopProject(safeId, null); }});
        allActions.push({ name: `Edit Project: ${safeName}`, searchKey: `editproject${safeName.toLowerCase()}`, icon: "✏️", action: () => { openEditModal(safeId); }});
    });
...
function getOrCreateTerminal(id, name) {
    ...
    searchBtn.onclick = () => {
        if (searchInput.classList.contains('hidden')) {
            ...
            searchInput.value = '';
            if (terminalPool[id].searchAddon) {
        ...
        term.onData(data => {
            for (let i = 0; i < data.length; i++) {
                ...
                if (code === 13 || code === 10) { 
                    ...
                    }).catch(err => console.error("Terminal send failed:", err));
    ...
    socket.onmessage = (e) => {
        try {
                ...
                statsArray.forEach(stat => {
                    ...
                    if (stat.status === 'running') {
                        ...
                        if (!terminalPool[stat.id]) {
                            ...
                            if (p) {
                                getOrCreateTerminal(p.id || p.ID, p.name || p.Name);
                            }
                        }
                    } else {
            ...
            if (msg.id === 'system') {
                ...
                projName = p ? (p.name || p.Name || msg.id) : msg.id;
            }
...
function formatWorkspacePath(path) {
    ...
    return path.substring(0, 3) + '...' + path.substring(path.length - 15);
}
...
function switchView(viewName) {
    ...
    if(!dashboardView || !settingsView || !viewTitle || !addBtn) return;
...
function applySelectionStyles() {
    ...
    Array.from(tbody.children).forEach(tr => {
        ...
        if (selectedProjectIds.has(id)) {
            ...
            tr.classList.remove('bg-[#0f111a]/30', 'hover:bg-gray-800/40');
        } else {
            tr.classList.remove('bg-indigo-500/10', 'shadow-[inset_4px_0_0_rgba(99,102,241,1)]');
...
async function loadProjects() {
    try {
        ...
        if (!text || text.trim() === '') {
            ...
            cachedProjects = JSON.parse(text) || [];
        }
    } catch (error) {
        console.error("[UI] Failed to load projects:", error);
        showToast("Error loading projects. Check console.", "error");
        cachedProjects = [];
    } finally {
        if (!Array.isArray(cachedProjects)) cachedProjects = [];
        renderSidebarTags(cachedProjects);
        renderProjects();
    }
}

function getActionButtonsHTML(pId, pName) {
    return `
        <div class="flex items-center gap-2 border-r border-gray-800/60 pr-3">
            <button onclick="startProject('${pId}', '${pName}', this)" class="btn-action w-16 bg-emerald-600/90 hover:bg-emerald-500 text-white py-1.5 rounded-lg shadow-[0_0_10px_rgba(16,185,129,0.2)] text-xs font-bold text-center transition-colors">Start</button>
            <button onclick="restartProject('${pId}', '${pName}', this)" class="btn-action w-16 bg-blue-600/90 hover:bg-blue-500 text-white py-1.5 rounded-lg shadow-[0_0_10px_rgba(59,130,246,0.2)] text-xs font-bold text-center transition-colors">Restart</button>
        ...
        <div class="flex items-center gap-1.5 ml-2">
            <button onclick="openEnvModal('${pId}')" class="btn-action bg-gray-800 hover:bg-teal-500 text-gray-400 hover:text-white p-1.5 rounded-lg transition-colors hover:shadow-[0_0_10px_rgba(20,184,166,0.3)]" title="Environment (.env)">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z"></path></svg>
            </button>
            <button onclick="openEditModal('${pId}')" class="btn-action bg-gray-800 hover:bg-indigo-600 text-gray-400 hover:text-white p-1.5 rounded-lg transition-colors hover:shadow-[0_0_10px_rgba(79,70,229,0.3)]" title="Edit">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
            </button>
            <button onclick="openDeleteModal('${pId}')" class="btn-action bg-gray-800 hover:bg-red-600 text-gray-400 hover:text-white p-1.5 rounded-lg transition-colors hover:shadow-[0_0_10px_rgba(225,29,72,0.3)]" title="Delete">
...
function renderProjects() {
    ...
    const filteredProjects = currentTagFilter ? cachedProjects.filter(p => {
        const t = p.tag || p.Tag;
        return t && t.toLowerCase() === currentTagFilter.toLowerCase();
    }) : cachedProjects;
    ...
    filteredProjects.forEach((p, index) => {
        ...
        const pId = p.id || p.ID || String(Math.random());
        const pName = p.name || p.Name || "Unknown";
        const pPath = p.path || p.Path || "Unknown Path";
        const pTag = p.tag || p.Tag || "";
        const pStatus = p.status || p.Status || "stopped";
        const pAutoStart = p.auto_start !== undefined ? p.auto_start : (p.AutoStart || false);
        const pAutoRestart = p.auto_restart !== undefined ? p.auto_restart : (p.AutoRestart || false);
        ...
        tr.addEventListener('contextmenu', (e) => {
            showContextMenu(e, pId, pName, pStatus);
        });
        ...
        const tagBadge = pTag ? `<span class="ml-3 inline-flex items-center gap-1 px-2.5 py-0.5 bg-gray-800 text-indigo-300 text-xs font-bold rounded-full border border-indigo-500/30 shadow-sm whitespace-nowrap"><span class="text-indigo-500 opacity-80 font-black">#</span>${pTag}</span>` : '';
        const autoBadge = pAutoStart ? `<span class="ml-2 text-emerald-400 drop-shadow-md hover:scale-110 transition-transform cursor-help" title="Auto-Start Enabled">⚡</span>` : '';
        const watchdogBadge = pAutoRestart ? `<span class="ml-1 text-amber-400 drop-shadow-md hover:scale-110 transition-transform cursor-help" title="Auto-Restart Enabled">🛡️</span>` : '';
        
        const firstLetter = pName.length > 0 ? pName.charAt(0).toUpperCase() : '?';
        ...
        tr.innerHTML = `
            ...
            <td class="p-5 text-sm text-gray-500 font-mono text-xs truncate max-w-xs group-hover:text-gray-400 transition-colors" title="${pPath}">${pPath}</td>
            ...
            <td class="p-5">
                <div class="flex items-center justify-end gap-3 whitespace-nowrap opacity-40 group-hover:opacity-100 transition-opacity duration-300">
                    ${getActionButtonsHTML(pId, pName)}
                </div>
            </td>
        `;
...
function renderSidebarTags(projects) {
    ...
    availableTags = [...new Set(projects.map(p => p.tag || p.Tag).filter(t => t && t.trim() !== ''))];
    ...
    availableTags.forEach(tag => {
        ...
        tagList.innerHTML += `<button id="btn-filter-${tag}" onclick="setFilter('${tag}')" class="tag-filter-btn w-full text-left px-4 py-1.5 rounded-md text-sm transition-colors border ${isActive ? 'bg-indigo-500/20 text-indigo-400 border-indigo-500/30' : 'border-transparent text-gray-400 hover:bg-gray-800/50'}"># ${tag}</button>`;
    });
}
...
// ==========================================
// BULK (TOPLU) SİLME İŞLEMLERİ
// ==========================================

function confirmBulkDelete() {
    let count = selectedProjectIds.size;
    if (count === 0 && currentTagFilter) {
        count = cachedProjects.filter(p => {
            const t = p.tag || p.Tag;
            return t && t.toLowerCase() === currentTagFilter.toLowerCase();
        }).length;
    }
...
async function executeBulkDelete() {
    ...
    if (selectedProjectIds.size > 0) {
        ...
        idsToProcess = cachedProjects.filter(p => {
            const t = p.tag || p.Tag;
            return t && t.toLowerCase() === currentTagFilter.toLowerCase();
        }).map(p => p.id || p.ID);
    }
    ...
    const deleteFilesCheckbox = document.getElementById('bulkDeleteFilesFromDisk');
    const deleteFiles = deleteFilesCheckbox ? deleteFilesCheckbox.checked : false;
...
// ==========================================
// DİĞER AJAX VE API İŞLEMLERİ
// ==========================================

async function submitNewProject(e) {
    e.preventDefault();
    const btn = e.target.querySelector('button[type="submit"]');
    const modeObj = document.querySelector('input[name="sourceMode"]:checked');
    const mode = modeObj ? modeObj.value : 'local';
    ...
    try {
        if (mode === 'local') {
            ...
            const useWsObj = document.getElementById('useWorkspace');
            const useWs = useWsObj ? useWsObj.checked : false;
...
function openEditModalWindow(id) {
    ...
    document.getElementById('editProjId').value = p.id || p.ID;
    document.getElementById('editProjName').value = p.name || p.Name;
    document.getElementById('editProjPath').value = p.path || p.Path;
    document.getElementById('editProjCmd').value = p.command || p.Command || '';
    document.getElementById('editProjTag').value = p.tag || p.Tag || '';
    
    const interCheck = document.getElementById('editProjInteractive');
    if (interCheck) interCheck.checked = p.interactive !== undefined ? p.interactive : (p.Interactive || false);
    
    const autoCheck = document.getElementById('editProjAutoStart');
    if (autoCheck) autoCheck.checked = p.auto_start !== undefined ? p.auto_start : (p.AutoStart || false);
    
    const resCheck = document.getElementById('editProjAutoRestart');
...
async function executeDelete() {
    ...
    const delFilesCb = document.getElementById('deleteFilesFromDisk');
    const delFiles = delFilesCb ? delFilesCb.checked : false;
    
    try {
        const res = await fetch(`/api/projects/delete?id=${projectToDelete}&remove_files=${delFiles}`, { method: 'DELETE' });
        ...
        toggleButtonLoading(btn, false, originalHTML);
        if (delFilesCb) delFilesCb.checked = false;
    }
}
...
function handleDragEnd() {
    ...
    document.querySelectorAll('#project-list tr').forEach(r => {
        r.classList.remove('border-t-2', 'border-indigo-500');
    });
}
...
// ==========================================
// ENV MODAL LOGIC (ORTAM DEĞİŞKENLERİ)
// ==========================================
...
async function openEnvModal(id) {
    ...
    try {
        ...
        if (textArea) textArea.value = "";
        showToast("Network error loading .env", "error");
    }
}
...
async function submitEnv(e) {
    ...
    try {
        ...
        showToast("Server error saving .env.", "error");
    } finally {
        toggleButtonLoading(btn, false, originalHTML);
    }
}
...
// ==========================================
// SYSTEM SETTINGS & FOLDER SELECTION
// ==========================================
...
function toggleSourceMode() {
    ...
    if (mode === 'local') {
        ...
        if (localWrapper) localWrapper.classList.remove('hidden');
        if (githubWrapper) githubWrapper.classList.add('hidden');
        
        if (projName) projName.required = true;
        if (projPath) projPath.required = true;
        if (repoUrl) repoUrl.required = false;
    } else {
        if (slider) slider.style.transform = 'translateX(100%)';
        if (localWrapper) localWrapper.classList.add('hidden');
        if (githubWrapper) githubWrapper.classList.remove('hidden');
        
        if (projName) projName.required = false;
        if (projPath) projPath.required = false;
        if (repoUrl) repoUrl.required = true;
    }
}